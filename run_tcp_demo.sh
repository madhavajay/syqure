...
HOST_EXAMPLE_DIR="${HOST_EXAMPLE_DIR:-$(pwd)/example}"
PLATFORM="${PLATFORM:-}"
...
docker run -d --rm --name syqure-cp0 --network "$NETWORK_NAME" \
  ...
  "$IMAGE_NAME" syqure "$EXAMPLE_PATH" -- 0 >/dev/null

docker run -d --rm --name syqure-cp1 --network "$NETWORK_NAME" \
  --ip "$CP1_IP" \
  "${platform_args[@]}" "${common_env[@]}" "${common_mount[@]}" \
  "$IMAGE_NAME" syqure "$EXAMPLE_PATH" -- 1 >/dev/null

docker run -d --rm --name syqure-cp2 --network "$NETWORK_NAME" \
  --ip "$CP2_IP" \
  "${platform_args[@]}" "${common_env[@]}" "${common_mount[@]}" \
  "$IMAGE_NAME" syqure "$EXAMPLE_PATH" -- 2 >/dev/null
